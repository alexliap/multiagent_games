...
class Agent:
    def __init__(self, environment_size: tuple, possible_tile_states: tuple, actions: tuple):
        self.no_of_tiles=environment_size[0]*environment_size[1]
        # get state to row mapping
        self.state_row_mapping=get_state_mapping(environment_size, possible_tile_states)
    ...
    def action(self, state: tuple):
        ...
        if random_num > self.epsilon:
            ...
            best_index=np.argmax(self.q_table[self.state_row_mapping[str(state)]][viable_actions])
            ...
            action_dict={'tile': random.choice(viable_actions) // 2, 
